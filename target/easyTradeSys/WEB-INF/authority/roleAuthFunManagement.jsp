...
<script type="text/javascript">
	...
	function refresh(currPage)
	{
		...
		$.ajax({
            type: "POST",
            url:"${pageContext.request.contextPath }/"+module+"/find?currPage="+currPage+"&user_no="+user_no,
            ...
            success:function(data){
	    			...
	    			$.each(rows,function(index,d){
	    				...
	    				var checked = d.active == 1 ? "checked" : "";
	    				// 拼接联合主键的ID
	    				var idObj = d.role_no+"-"+d.module_no+"-"+d.fun_flag;
	    				...
	    				var rowElements="<tr>"+
	    									...
	    									"<td>"+d.fun_flag+"</td>"+
	    									"<td>"+d.fun_desc+"</td>"+
	    									"<td><input type='checkbox' name='active' disabled='disabled' "+checked+"/></td>"+
	    								"</tr>";
	    				// 向表格添加行
	...
	function save()
	{
		...
		if(actionType == "add")
		{
			...
			$(".addRow").each(function(index,element){
				...
				var active = $(element).find("input[name='active']").is(":checked");
				// 封装成JSON对象
				var obj = {
				 "role_no":role_no,
				 "module_no":module_no,
		...
		else if(actionType == "update")
		{
			...
			$(checkedItems).each(function(index,element)
			{
				...
				// 获取其他列的
				var active = $(tr).find("input[name='active']").is(":checked");
				// 封装成JSON对象
				var obj = {
				 "role_no":role_no,
				 "module_no":module_no,
				...
				$.messager.confirm('信息提示','确定要删除记录？', function(result){
					...
					if(result){
						...
						$(checkedItems).each(function(index,element){
							...
							var obj = {"role_no":id_arr[0],"module_no":id_arr[1],"fun_flag":id_arr[2]};
							ids.push(obj);	
						});
						// 将标识变为delete
						actionType = "delete";
	...
	function openAdd(){
		...
		var rowElements="<tr class='addRow'>"+
							...
							"<td><input type='text' name='role_no' class='role' onclick='pop(this)' /></td>"+
							"<td name='role_name'></td>"+
							"<td><input type='text' name='module_no' class='module' onclick='pop(this)' /></td>"+
							"<td name='module_name'></td>"+
							"<td><input type='text' name='fun_flag' related-column='module_no' class='moduleFuns' onclick='pop(this)'/></td>"+
							"<td name='fun_desc'></td>"+
		...
		else
		{
			...
			$(checkedItems).each(function(index,element)
			{
				...
				$(td).each(function(index,element)
				{
					...
					var tr = $(element).parent().parent();
					// 获取激活checkbox
					var active = $(tr).find("input[name='active']");
					// 设置为可修改
