    	...
    	<!-- Begin of toolbar -->
	       <div id="wu-toolbar">
				...
				<div class="wu-toolbar-search">
	                <label>入库单号：</label><input id="in_list_no" class="wu-text" style="width:100px">
	                ...
	                <label>入库日期起：</label><input id="in_date_start" class="easyui-datebox" style="width:100px">
	                <label>入库日期止：</label><input id="in_date_end" class="easyui-datebox" style="width:100px">
...
<script type="text/javascript">
	...
	function refresh(currPage)
	{
		...
		var condition = [
			...
			{"condition_key":"in_date_from","condition_value":$(".combo-value:eq(0)").val(),"condition_type":"dateFrom"},
			{"condition_key":"in_date_to","condition_value":$(".combo-value:eq(1)").val(),"condition_type":"dateTo"}
		];
		...
		$.ajax({
            ...
            success:function(data){
	    			...
	    			$.each(rows,function(index,d){
	    				// 拼接联合主键的ID
	    				var idObj = d.in_list_no+"-"+d.prod_no;
	    				// 要添加的行内容
	    				var rowElements="<tr>"+
	...
	function checkExist(obj)
	{
		...
		{
			$.post("${pageContext.request.contextPath }/inventoryInfo/exist?prod_no="+objVal,function(data){
				var exist = $.parseJSON(data).exist;
				if(exist == 0)
				{
					$(obj).val("产品编号在库存信息不存在!");
					$(obj).css("color","red");
					$(obj).addClass("invalid");
				}
				else
				{
					if($(obj).attr("class") == "invalid" )
					{
		...
		else
		{
			...
			if(actionType == "add")
			{
				...
				$(".addRow").each(function(index,element){
					// 找到各个元素的value
					var in_list_no = $(element).find("input[name='in_list_no']").val();
				...
				$.messager.confirm('信息提示','确定要删除记录？', function(result){
					...
					if(result){
						...
						$(checkedItems).each(function(index,element){
							...
							var obj = {"in_list_no":id_arr[0],"prod_no":id_arr[1]};
							ids.push(obj);	
						});
						// 将标识变为delete
						actionType = "delete";
	...
	function openAdd(){
		$.post("${pageContext.request.contextPath }/"+module+"/getId",function(data){
			// 获取服务器返回的数据
			var in_list_no = $.parseJSON(data).in_list_no;
			// 获取系统日期字符串
			var sysdate = in_list_no.substring(prefix.length,prefix.length+7);
			// 将数据中的数字截取出来,不包含0
			var numPart = parseInt(in_list_no.replace(/[^1-9]/ig,""));
			// 如果之前已经添加过行
			if($(".addRow").length != 0)
			{
				// 找出添加的最后一行
				...
				// 找出添加的最后一行中的input的值
				var last_in_list_no = $(lastTr).find("td input[name='in_list_no']").val();
				// 将数据中的数字截取出来,不包含0 
				var maxNo = parseInt(last_in_list_no.replace(/[^1-9]/ig,""));
			...
			var rowElements="<tr class='addRow'>"+
								...
								"<td><input type='text' name='in_list_no' value='"+in_list_no+"'/></td>"+
