    	...
    	<!-- Begin of toolbar -->
	       <div id="wu-toolbar">
				...
				<div class="wu-toolbar-search">
	                <label>出库单号：</label><input id="out_list_no" class="wu-text" style="width:100px">
	                ...
	                <label>出库日期：</label><input id="out_date_from" class="easyui-datebox" style="width:100px">
	                <label>出库日期：</label><input id="out_date_to" class="easyui-datebox" style="width:100px">
...
<script type="text/javascript">
	...
	function refresh(currPage)
	{
		...
		var condition = [
			{"condition_key":"out_list_no","condition_value":$("#out_list_no").val(),"condition_type":"string"},
			...
			{"condition_key":"out_date_from","condition_value":$(".combo-value:eq(0)").val(),"condition_type":"string"},
			{"condition_key":"out_date_to","condition_value":$(".combo-value:eq(1)").val(),"condition_type":"string"}
		];
		...
		$.ajax({
            ...
            success:function(data){
	    			...
	    			$.each(rows,function(index,d){
	    				// 拼接联合主键的ID
	    				var idObj = d.out_list_no+"-"+d.prod_no;
	    				// 要添加的行内容
	    				var rowElements="<tr>"+
	    									...
	    									"<td>"+d.out_qty+"</td>"+
	    									"<td>"+d.out_date+"</td>"+
	    									"<td>"+d.pur_order_no+"</td>"+
	    									"<td>"+d.int_order_no+"</td>"+
	    								"</tr>";
	    				// 向表格添加行				
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                $(data.rows).each(function(index,element){
                    // 封装成JSON对象
                    var obj = {
                        "out_list_no":element.out_list_no,
                        ...
                        "int_order_no":element.int_order_no
                    };
                    // 将JSON对象添加到数组
                    objs.push(obj);
                });
	...
	function save()
	{
		...
		if(actionType == "add")
		{
			...
			$(".addRow").each(function(index,element){
				// 找到各个元素的value
				var out_list_no = $(element).find("input[name='out_list_no']").val();
				var prod_no = $(element).find("input[name='prod_no']").val();
				var out_qty = $(element).find("input[name='out_qty']").val();
				// 封装成JSON对象
				var obj = {
				 "out_list_no":out_list_no,
				 "prod_no":prod_no,
				 "out_qty":out_qty
				};
				// 将JSON对象添加到数组
		...
		else if(actionType == "update")
		{
			...
			$(checkedItems).each(function(index,element)
			{
				...
				var out_list_no = ids[0];
				var prod_no = ids[1];
				// 获取行
				var tr = $(element).parent().parent();
				// 获取其他列的
				var out_qty = $(tr).find("input[name='out_qty']").val();
				// 封装成JSON对象
				var obj = {
				 "out_list_no":out_list_no,
				 "prod_no":prod_no,
				...
				$.messager.confirm('信息提示','确定要删除记录？', function(result){
					...
					if(result){
						...
						$(checkedItems).each(function(index,element){
							...
							var obj = {"out_list_no":id_arr[0],"prod_no":id_arr[1]};
							ids.push(obj);	
						});
						// 将标识变为delete
						actionType = "delete";
	...
	function openAdd(){
		$.post("${pageContext.request.contextPath }/"+module+"/getId",function(data){
			// 获取服务器返回的数据
			var out_list_no = $.parseJSON(data).out_list_no;
			// 获取系统日期字符串
			var sysdate = out_list_no.substring(prefix.length,prefix.length+8);
			// 将数据中的数字截取出来,不包含0
			var numPart = parseInt(out_list_no.replace(/[^1-9]/ig,""));
			// 如果之前已经添加过行
			if($(".addRow").length != 0)
			{
				// 找出添加的最后一行
				...
				// 找出添加的最后一行中的input的值
				var last_out_list_no = $(lastTr).find("td input[name='out_list_no']").val();
				// 将数据中的数字截取出来,不包含0 
				var maxNo = parseInt(last_out_list_no.replace(/[^1-9]/ig,""));
			...
			var rowElements="<tr class='addRow'>"+
								...
								"<td><input type='text' name='out_list_no' value='"+out_list_no+"'/></td>"+
								"<td><input type='text' name='prod_no' class='inventoryInfo' onclick='pop(this)' /></td>"+
								"<td name='prod_name'></td>"+
								"<td><input type='text' name='out_qty' value='0'/></td>"+
								"<td name='out_date'></td>"+
								"<td><input type='text' name='pur_order_no' /></td>"+
								"<td><input type='text' name='int_order_no' /></td>"+
							"</tr>";
			// 向表格添加行				
			$("#wu-datagrid").append(rowElements);
			// 设置焦点
