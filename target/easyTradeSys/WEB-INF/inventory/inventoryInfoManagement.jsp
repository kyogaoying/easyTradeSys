...
<script type="text/javascript">
		...
		$.ajax({
            ...
            success:function(data){
	    			...
	    			$.each(rows,function(index,d){
	    				// 要添加的行内容
	    				var rowElements="<tr>"+
	    									"<td><input type='checkbox' id='"+d.prod_no+"' name='checkedItem' /></td>"+
	    									"<td>"+d.prod_no+"</td>"+
	    									"<td>"+d.prod_name+"</td>"+
	...
	function save()
	{
		...
		if(actionType == "add")
		{
			...
			$(".addRow").each(function(index,element){
				...
				var prod_name = $(element).find("input[name='prod_name']").val();
				var available_qty = $(element).find("input[name='available_qty']").val();
				var inventory_loc = $(element).find("input[name='inventory_loc']").val();
				// 封装成JSON对象
				var obj = {
				 "prod_no":prod_no,
				 "prod_name":prod_name,
		...
		else if(actionType == "update")
		{
			...
			{
				// 获取checkbox的id,即prod_no
				var prod_no = $(element).attr("id");
				// 获取行
				var tr = $(element).parent().parent();
				// 获取其他列的
				var prod_name = $(tr).find("input[name='prod_name']").val();
				var available_qty = $(tr).find("input[name='available_qty']").val();
				var inventory_loc = $(tr).find("input[name='inventory_loc']").val();
				// 封装成JSON对象
				var obj = {
				 "prod_no":prod_no,
				 "prod_name":prod_name,
	...
	function openAdd(){
		$.post("${pageContext.request.contextPath }/"+module+"/getId",function(data){
			// 获取服务器返回的数据
			var prod_no = $.parseJSON(data).prod_no;
			// 将数据中的数字截取出来,不包含0
			var numPart = parseInt(prod_no.replace(/[^1-9]/ig,""));
			// 如果之前已经添加过行
			if($(".addRow").length != 0)
			{
				// 找出添加的最后一行
				...
				// 找出添加的最后一行中的input的值
				var last_prod_no = $(lastTr).find("td input[name='prod_no']").val();
				// 将数据中的数字截取出来,不包含0 
				var maxNo = parseInt(last_prod_no.replace(/[^1-9]/ig,""));
				// 如果前端生成的数字比服务器返回的数字大,则在前端直接加1,然后生成单号
				if(numPart < maxNo + 1)
				{
					prod_no = prefix + addPrefixZero(maxNo+1,flowLength);
				}
			}
	    		// 将要添加的行内容
			var rowElements="<tr class='addRow'>"+
								"<td><input type='checkbox' name='checkedItem' disabled='disabled' checked/></td>"+
								"<td><input type='text' name='prod_no' value='"+prod_no+"'/></td>"+
								"<td><input type='text' name='prod_name' /></td>"+
								"<td><input type='text' name='available_qty' value='0'/></td>"+
								"<td><input type='text' name='inventory_loc' /></td>"+
							"</tr>";
			// 向表格添加行				
			$("#wu-datagrid").append(rowElements);
			// 设置焦点
	...
	function openEdit(){
		...
		else
		{
			...
			var names = ["prod_name","available_qty","inventory_loc"];
			var name_index = 0;
			// 禁用所有的checkbox
