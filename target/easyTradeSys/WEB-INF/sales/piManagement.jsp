    ...
    <div data-options="region:'center',border:false">
        ...
        <div id="wu-toolbar">
            <div class="wu-toolbar-button">
                ...
                <a href="#" class="easyui-linkbutton" iconCls="icon-arrow-down" onclick="generateCI()" plain="true" >生成CI</a>
                <a href="#" class="easyui-linkbutton" iconCls="icon-arrow-down" onclick="generateCustomCI()" plain="true" >生成报关发票</a>
                <a href="#" class="easyui-linkbutton" iconCls="icon-arrow-down" onclick="generatePL()" plain="true" >生成PL</a>
                <a href="#" class="easyui-linkbutton" iconCls="icon-arrow-down" onclick="generateCustomPL()" plain="true" >生成报关箱单</a>
            </div>
            <div class="wu-toolbar-search">
                <label>PI编号：</label><input class="wu-text" style="width:100px" id="search_pi_no" name="searchArea">&nbsp;
        ...
        <table id="wu-datagrid" toolbar="#wu-toolbar">
			...
			<c:forEach items="${allInfos}" var="info">
				<tr class="tRow">
					...
					<td>
						<a class="piNo" href="queryPi?pi_no=${info[0]}">
							${info[0]}
						</a>
					</td>
					<td>${info[1]}</td>
        ...
        <div id="turnPages">
        	<button class="turnPage" id="firstPage" onClick="javascript:void(window.location.href='${pageContext.request.contextPath }/piManagement/allInfos?page=1')">首   页</button>
        	<button class="turnPage" id="prePage" onClick="javascript:void(window.location.href='${pageContext.request.contextPath }/piManagement/allInfos?page=${sessionScope.page-1}')">上一页</button>
        	<button class="turnPage" id="nextPage" onClick="javascript:void(window.location.href='${pageContext.request.contextPath }/piManagement/allInfos?page=${sessionScope.page+1}')">下一页</button>
        	<button class="turnPage" id="lastPage" onClick="javascript:void(window.location.href='${pageContext.request.contextPath }/piManagement/allInfos?page=${sessionScope.totalPages }')">末   页</button>
        </div>
    </div>
</div>
...
<script type="text/javascript">
	...
	*/
	function refresh(){
		$.post("${pageContext.request.contextPath }/piManagement/queryAfterDML?page=${sessionScope.page}",function(allRecords){
	...
	*/
	function findOneSeveralTimes(currPage){
		$.ajax({
	            type: "POST",
	            url:"${pageContext.request.contextPath }/piManagement/queryOne?page="+currPage,
	            ...
	            success:function(data){
	            	...
	            	$.each(data,function(index,element){
	            		var rowElements="<tr class='tRow'>"+
							"<td><input id='"+element[0]+"' type='checkbox' name='checkedItem'/></td>"+
							"<td>"+
							"<a class='piNo' href='queryPi?pi_no="+element[0]+"'>"+element[0]+"</a>"+
							"</td>"+
				    		"<td>"+element[1]+"</td>"+
				    		"<td>"+element[2]+"</td>"
				    		"</tr>";
	...
	*/
    function findOneFirstTime() {
		...
		var arr_searchConditions_json=[
			{"condition_key":"pi_no","condition_value":$("#search_pi_no").val(),"condition_type":"string"},
		...
		if(($("#search_pi_no").val()==null || $("#search_pi_no").val()=="") 
		...
		else
		{
			$.ajax({
	            ...
	            url:"${pageContext.request.contextPath }/piManagement/queryOne?page=1",
	            ...
	            success:function(data){
	    			...
	    			$.ajax({
	    	            type: "POST",
	    				//一开始查询,肯定是从第一页开始查,所以这里的page参数是第一页
	    	            url:"${pageContext.request.contextPath }/piManagement/getNewTotalPages",
	    	            data: JSON.stringify(arr_searchConditions_json),
	    	            dataType:"json",
	    	            contentType:"application/json;charset=UTF-8",
	    	            success:function(data){
	...
	*/
	function remove(){
		...
		$.messager.confirm('信息提示','确定要删除单据吗？', function(result){
			if(result){
				//创建存放piInfo json对象的数组
				var arr_pi_no_json=[];
				//获取待删除行checkbox的id，id即pi_no
				$("input[name='checkedItem']:checked").each(function(){
						var checkbox_id=$(this).attr("id");
						var pi_no_json={"pi_no":checkbox_id};
    ...
    function generateCI(){
        //一条都没选
        if($("input[name='checkedItem']:checked").length==0){
            $.messager.alert('信息提示','请选中至少一行！','info');
            return false;//退出方法
        }
        //选了多于一行
        ...
        //发送AJAX，查询是否有下推内部订单
        $.get("${pageContext.request.contextPath }/shipmentManagement/checkIfLocked?pi_no="+pi_no,function (data) {
            if(data.result=="success"){
            window.location.href="${pageContext.request.contextPath }/shipmentManagement/generateInvoice?pi_no="+pi_no;
            }
            else if(data.result=="fail"){
                $.messager.alert('信息提示','此张合同还未解锁，无法生成CI！','info');
            }
        });
    }
    function generateCustomCI(){
        //一条都没选
        if($("input[name='checkedItem']:checked").length==0){
            $.messager.alert('信息提示','请选中至少一行！','info');
            return false;//退出方法
        ...
        $.get("${pageContext.request.contextPath }/shipmentManagement/checkIfLocked?pi_no="+pi_no,function (data) {
            if(data.result=="success"){
                window.location.href="${pageContext.request.contextPath }/shipmentManagement/generateCustomInvoice?pi_no="+pi_no;
            }
            else if(data.result=="fail"){
                $.messager.alert('信息提示','此张合同还未解锁，无法生成报关发票！','info');
            }
        });
    }
    function generatePL(){
        ...
        $.get("${pageContext.request.contextPath }/shipmentManagement/checkIfLocked?pi_no="+pi_no,function (data) {
            if(data.result=="success"){
                window.location.href="${pageContext.request.contextPath }/shipmentManagement/generatePackingList?pi_no="+pi_no;
            }
            else if(data.result=="fail"){
                $.messager.alert('信息提示','此张合同还未解锁，无法生成PL！','info');
            }
        });
    }
    function generateCustomPL(){
        //一条都没选
        if($("input[name='checkedItem']:checked").length==0){
            $.messager.alert('信息提示','请选中至少一行！','info');
            return false;//退出方法
        ...
        $.get("${pageContext.request.contextPath }/shipmentManagement/checkIfLocked?pi_no="+pi_no,function (data) {
            if(data.result=="success"){
                window.location.href="${pageContext.request.contextPath }/shipmentManagement/generateCustomPackingList?pi_no="+pi_no;
            }
            else if(data.result=="fail"){
                $.messager.alert('信息提示','此张合同还未解锁，无法生成PL！','info');
            }
