    	...
    	<!-- Begin of toolbar -->
	       <div id="wu-toolbar">
				...
				<div class="wu-toolbar-search">
	                <label>装箱单号：</label><input id="packing_list_no" class="wu-text" style="width:100px">
	                <label>客户名称：</label><input id="customer_name" class="wu-text" style="width:100px">
	                <label>装箱单日期起：</label><input id="packing_list_date_from" class="easyui-datebox" style="width:100px">
	                <label>装箱单日期止：</label><input id="packing_list_date_to" class="easyui-datebox" style="width:100px">
...
<script type="text/javascript">
	...
	function refresh(currPage)
	{
		...
		var condition = [
			{"condition_key":"packing_list_no","condition_value":$("#packing_list_no").val(),"condition_type":"string"},
			{"condition_key":"customer_name","condition_value":$("#customer_name").val(),"condition_type":"string"},
			{"condition_key":"packing_list_date_from","condition_value":$(".combo-value:eq(0)").val(),"condition_type":"dateFrom"},
			{"condition_key":"packing_list_date_to","condition_value":$(".combo-value:eq(1)").val(),"condition_type":"dateTo"}
		];
		...
		$.ajax({
            ...
            success:function(data){
	    			...
	    			$.each(rows,function(index,d){
	    				// 拼接联合主键的ID
	    				var idObj = d.packing_list_no;
	    				var url = "${pageContext.request.contextPath }/"+module+"/findById?packing_list_no="+idObj+"&des=queryCustomPackingList";
	    				// 要添加的行内容
	    				var rowElements="<tr>"+
	    									"<td><input type='checkbox' id='"+idObj+"' name='checkedItem' /></td>"+
	    									"<td>"+
											"<a href='"+url+"'>"+d.packing_list_no+"</a>"+"</td>"+
	    									"<td>"+d.customer_name+"</td>"+
	    									"<td>"+d.packing_list_date+"</td>"+
	    								"</tr>";
	    				// 向表格添加行				
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                $(data.rows).each(function(index,element){
                    // 封装成JSON对象
                    var obj = {
                        "packing_list_no":element.packing_list_no,
                        "customer_name":element.customer_name,
                        "packing_list_date":element.packing_list_date
                    };
                    // 将JSON对象添加到数组
                    objs.push(obj);
                });
		...
		else
		{
			...
			if(actionType == "add")
			{
				...
				$(".addRow").each(function(index,element){
					// 找到各个元素的value
					var packing_list_no = $(element).find("input[name='packing_list_no']").val();
				...
				$.messager.confirm('信息提示','确定要删除记录？', function(result){
					...
					if(result){
						...
						$(checkedItems).each(function(index,element){
							...
							var obj = {"packing_list_no":id_arr[0],"prod_no":id_arr[1]};
							ids.push(obj);	
						});
						// 将标识变为delete
						actionType = "delete";
	...
	function openAdd(){
		$.post("${pageContext.request.contextPath }/"+module+"/getId",function(data){
			// 获取服务器返回的数据
			var packing_list_no = $.parseJSON(data).packing_list_no;
			// 获取系统日期字符串
			var sysdate = packing_list_no.substring(prefix.length,prefix.length+7);
			// 将数据中的数字截取出来,不包含0
			var numPart = parseInt(packing_list_no.replace(/[^1-9]/ig,""));
			// 如果之前已经添加过行
			if($(".addRow").length != 0)
			{
				// 找出添加的最后一行
				...
				// 找出添加的最后一行中的input的值
				var last_packing_list_no = $(lastTr).find("td input[name='packing_list_no']").val();
				// 将数据中的数字截取出来,不包含0 
				var maxNo = parseInt(last_packing_list_no.replace(/[^1-9]/ig,""));
			...
			var rowElements="<tr class='addRow'>"+
								...
								"<td><input type='text' name='packing_list_no' value='"+packing_list_no+"'/></td>"+
