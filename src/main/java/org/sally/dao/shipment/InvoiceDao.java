...
public class InvoiceDao extends BaseDao {
	...
	public String getID(){
		//生成发票号
		String  invoice_no=hibernateUtil.generateFlowId("V","invoice_no","INVOICE_TAB",3,true);
		return  invoice_no;
	}
	...
	public List<Invoice> refresh(String invoice_no){
		...
		String hql="FROM Invoice WHERE invoice_no=:invoice_no";
		List<Invoice> invoices= session.createQuery(hql).setParameter("invoice_no",invoice_no).list();
	...
	public List<Object[]> queryOne(List<Condition> conditions, int page) {
		...
		String hql_query="SELECT DISTINCT invoice_no, customer_name,invoice_date FROM Invoice WHERE 1=1";
		//将原始查询条件代入工具方法,拼接查询条件
		hql_query= hibernateUtil.conditionalHQL(hql_query, conditions,"invoice_date");
