...
public class RoleController
{
	...
	@ResponseBody
	public Object add(@RequestBody List<Role> roles)
	{
		ExecuteResult er = null;
		try
		{
			er = roleService.add(roles);
		}
		catch (Exception e)
		{
	...
	@ResponseBody
	public ExecuteResult delete(@RequestBody List<Role> roles)
	{
		ExecuteResult er = null;
		try
		{
			er = roleService.delete(roles);
		}
		catch (Exception e)
		{
			if(er == null) er = new ExecuteResult();
	...
	public String find(String currPage, @RequestBody List<Condition> conditions)
	{
		...
		{
			// 查询所有符合条件的用户集合
			List<Role> list = roleService.find(Integer.parseInt(currPage),conditions);
			// 查询符合条件的记录数
			long totalCount = roleService.getCount(conditions);
	...
	@ResponseBody
    public ExecuteResult update(@RequestBody List<Role> roles) throws Exception
	{
		ExecuteResult er = null;
		try
		{
			er = roleService.update(roles);
		}
		catch (Exception e)
		{
			er.setMessage(e.getLocalizedMessage());
	...
	public String findAll(@RequestBody List<Condition> conditions)
	{
		...
		{
			// 查询所有符合条件的集合
			List<Role> list = roleService.findAll(conditions);
			// 将数据转换为JSON
	...
	@ResponseBody
	public String export(String colNames,@RequestBody List<Role> list)
	{
